...
if ( ! class_exists(__NAMESPACE__ . '\Manifest') ) {
    ...
    class Manifest {
        ...
        public function manifest_post_type() {
            $labels = array(
              'name' => __('Pickup Orders', 'woo-topship-africa'),
              'singular_name' => __('Pickup Order', 'woo-topship-africa'),
              'all_items' => __('All Pickup Orders', 'woo-topship-africa'),
              'add_new_item' => __('Create New Pickup Order', 'woo-topship-africa'),
              'add_new' => __('Create', 'woo-topship-africa'),
            );
            register_post_type(
              'pk_manifest',
              array(
        ...
        public function manifest_post_types_statuses() {
            register_post_status(
              'open',
              array(
                'label' => __('Open ', 'woo-topship-africa'),
                'public' => is_admin(),
                /* translators: %s: label_count */
                'label_count' => _n_noop('Open <span class="count">(%s)</span>', 'Open <span class="count">(%s)</span>', 'woo-topship-africa'),
            ...
            register_post_status(
              'closed',
              array(
                'label' => __('Closed ', 'woo-topship-africa'),
                'public' => is_admin(),
                /* translators: %s: label_count */
                'label_count' => _n_noop('Closed <span class="count">(%s)</span>', 'Closed <span class="count">(%s)</span>', 'woo-topship-africa'),
                'post_type' => array( 'pk_manifest' ), // Define one or more post types the status can be applied to.
                'show_in_admin_all_list' => true,
                'show_in_admin_status_list' => true,
                'show_in_metabox_dropdown' => true,
        ...
        public function register_multi_manifest_orders( $bulk_actions ) {
            ...
            if ( version_compare($wp_version, '5.6.0', '>=') ) {
                ...
                $bulk_actions[$this->core->vendor_name][str_replace('wc_', '', $this->core->prefix) . '_add_to_manifest'] = __('Add to pickup order', 'woo-topship-africa');
            } else {
                $bulk_actions[str_replace('wc_', '', $this->core->prefix) . '_add_to_manifest'] = $this->core->vendor_name . ': ' . __('Add to pickup order', 'woo-topship-africa');
            }
        ...
        public function add_manifest_order_action( $actions ) {
            ...
            $actions[ $this->core->prefix . '_add_to_manifest' ] = __('Add to pickup order', 'woo-topship-africa');
            return $actions;
        }
        ...
        private function add_orders_to_manifest( $manifest, $order_ids ) {
            ...
            foreach ( $order_ids as $order_id ) {
                ...
                if ( ! empty($labels) ) {
                    ...
                    $success[] = sprintf(__('Order #%s added to pickup order', 'woo-topship-africa'), $order_id);
                } else {
                    /* translators: %s: order_number */
                    $errors[] = sprintf(__('Order #%s does not have shipment labels', 'woo-topship-africa'), $order_id);
                }
            }
        ...
        public function set_pk_manifest_columns( $columns ) {
            unset($columns['date']);
            $columns['orders'] = __('Orders', 'woo-topship-africa');
            $columns['status'] = __('Status', 'woo-topship-africa');
            $columns['pickup_time'] = __('Pickup time', 'woo-topship-africa');
            $columns['actions'] = __('Actions', 'woo-topship-africa');
                ...
                if ( $manifest->post_status == 'open' ) {
                  ...
                  if ( ! empty($current_orders) ) {
                    ...
                    <div id="manifest-id-<?php echo $manifest->ID; ?>" style="display:none;">
                        <p>
                            <?php _e('Select date and time between courier should pickup order', 'woo-topship-africa'); ?>
                        <table class = "call-courier-table">
                            <tr>
                                <th><?php _e('Date', 'woo-topship-africa'); ?></th>
                                <th><?php _e('Earliest time', 'woo-topship-africa'); ?></th>
                                <th><?php _e('Latest time', 'woo-topship-africa'); ?></th>
                                <th><?php _e('Additional information', 'woo-topship-africa'); ?></th>
                                <th></th>
                            </tr>
                            <tr>
                                ...
                                <td><input type = "button" class = "button manifest_action" data-id = "<?php echo $manifest->ID; ?>" value = "<?php _e('Submit', 'woo-topship-africa'); ?>"/></td>
                            </tr>
                        </table>
                        </p>
                    </div>

                    <a href="#TB_inline?&width=600&height=200&inlineId=manifest-id-<?php echo $manifest->ID; ?>" class = "button thickbox"><?php echo __('Place pickup order', 'woo-topship-africa'); ?></a>
                    <?php

                  } else {
                      echo __('No orders assigned', 'woo-topship-africa');
                  }
                } else if ( $manifest->post_status == 'closed' ) {
                  echo __('Already placed pickup order', 'woo-topship-africa');
                }
                break;
              case 'status':
        ...
        public function pk_manifest_call_courier() {
            try {
                ...
                if ( ! $date || ! $time_from || ! $time_to || ! $id ) {
                    echo json_encode(array( 'error' => __('Wrong data requested, please try again', 'woo-topship-africa') ));
                    wp_die();
                }
                if ( $manifest && $manifest->post_status == 'open' && $manifest->post_type == 'pk_manifest' ) {
                    ...
                    if ( empty($order_ids) ) {
                        echo json_encode(array( 'error' => __('Manifest has no orders assigned', 'woo-topship-africa') ));
                    } else {
                        $response = $this->make_call($date, $time_from, $time_to, $manifest, $order_ids, $additional_info);
                        if ( ! isset($response['status']) ) {
                            throw new \Exception(__('Wrong response.', 'woo-topship-africa'));
                        }
                    ...
                    echo json_encode(array( 'error' => __('Manifest not found or incorrect status', 'woo-topship-africa') ));
                }
            } catch ( \Exception $e ) {
        ...
        private function make_call( $date, $time_from, $time_to, $manifest, $order_ids, $additional_info ) {
            ...
            if ( ! $url ) {
                throw new \Exception(__('Order pickup URL not set.', 'woo-topship-africa'));
            }
            $transient_name = $this->core->prefix . '_access_token';
            $token = get_transient($transient_name);
            if ( ! $token ) {
                throw new \Exception(__('Token not found. Please check credentials', 'woo-topship-africa'));
            }
        ...
        */
        ...
        public function order_meta_box( $post ) {
            ...
            if ( ! $manifest_id ) {
                echo '<h4>' . esc_attr__('No manifest assigned', 'woo-topship-africa') . '</h4>';
                return;
            }
            ...
            ?>
            <h4><?php echo esc_attr__('Manifest ID', 'woo-topship-africa'); ?>: <?php echo $manifest_id; ?></h4>
            <h4><?php echo esc_attr__('Manifest status', 'woo-topship-africa'); ?>: <?php echo get_post_status($manifest); ?></h4>
            <hr/>
            <h4><?php echo esc_attr__('Assigned orders', 'woo-topship-africa'); ?>:</h4>
            <?php
            ...
            foreach ( $current_orders as $order_id ) {
                ...
                if ( $_order !== null ) {
                    ...
                    <li>
                        ...
                        <?php
                        echo empty($data) ? __('Shipment not ready', 'woo-topship-africa') : __('Shipment ready', 'woo-topship-africa');
                        ?>
                    </li>
                    <?php
                }

    ...
    class Check_Tool {
        ...
        public function __construct( Core $plugin ) {
            ...
            if ( strtolower($this->core->vendor_name) != $this->vendor_type ) {
                ...
                $this->page_title = __('Posti Shipping diagnostic tool', 'woo-topship-africa');
            } else {
                $this->page_title = __('Pakettikauppa diagnostic tool', 'woo-topship-africa');
            }
        ...
        public function render_check_tool() {
            ...
            <div class = "pakettikauppa-check-tool-content">
                <div class = "pakettikauppa-check-tool-line">
                    <b><?php _e('PHP version', 'woo-topship-africa'); ?></b> <?php echo $this->check_php_version(); ?>
                </div>
                <div class = "pakettikauppa-check-tool-line">
                    <b><?php _e('Curl extension', 'woo-topship-africa'); ?></b> <?php echo $this->check_curl(); ?>
                </div>
                <div class = "pakettikauppa-check-tool-line">
                    <b><?php _e('Wordpress version', 'woo-topship-africa'); ?></b> <?php echo $this->check_wordpress_version(); ?>
                </div>
                <div class = "pakettikauppa-check-tool-line">
                    <b><?php _e('WooCommerce version', 'woo-topship-africa'); ?></b> <?php echo $this->check_woocommerce_version(); ?>
                </div>
                <div class = "pakettikauppa-check-tool-line">
                    <b>
                        <?php
                        /* translators: %s: vendor_name */
                        printf(__('WooCommerce %s version', 'woo-topship-africa'), $this->core->vendor_name);
                        ?>
            ...
            <?php if ( isset($this->check_urls[$this->vendor_type]) ) : ?>
                <?php foreach ( $this->check_urls[ $this->vendor_type ] as $url ) : ?>
                        <div class = "pakettikauppa-check-tool-line">
                            <b><?php _e('Checking URL ', 'woo-topship-africa'); ?> <?php echo $url; ?></b> <?php echo $this->check_url($url); ?>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
                <div class = "pakettikauppa-check-tool-line">
                    <b><?php _e('API login information', 'woo-topship-africa'); ?></b> <?php echo $this->check_api_login(); ?>
                </div>
                <?php if ( $this->vendor_type !== 'pakettikauppa' ) : ?>
                <div class = "pakettikauppa-check-tool-line">
                    <b><?php _e('Cached token', 'woo-topship-africa'); ?></b> <?php echo $this->check_cached_token(); ?>
                </div>
                <?php endif; ?>
        ...
        private function check_curl() {
            if ( phpversion('curl') ) {
                return $this->render_success(__('Installed', 'woo-topship-africa'));
            } else {
                return $this->render_error(__('Not installed', 'woo-topship-africa'));
            }
        }
        ...
        private function check_current_version() {
            ...
            if ( $this->vendor_type === 'pakettikauppa' ) {
                if ( ! file_exists($this->core->dir . 'wc-pakettikauppa.php') ) {
                    return $this->render_error(__('Could not load plugin\'s file', 'woo-topship-africa'));
                }
            ...
            if ( ! is_array($plugin_data) || ! isset($plugin_data['Version']) ) {
                return $this->render_error(__('Could not read version', 'woo-topship-africa'));
            }
        ...
        private function check_url( $url ) {
            if ( ! phpversion('curl') ) {
                $this->render_error(__('Curl not installed', 'woo-topship-africa'));
            }
            $curl = curl_init($url);
            ...
            if ( $result !== false ) {
                ...
                if ( $status_code == 404 ) {
                    return $this->render_error(__('Not found', 'woo-topship-africa'));
                } else {
                    return $this->render_success(__('OK', 'woo-topship-africa'));
                }
            } else {
        ...
        private function check_api_login() {
            ...
            if ( $api_check['api_good'] ) {
                ...
                if ( ! empty($api_check['code']) ) {
                    $msg .= ' <i>(' . __('Code', 'woo-topship-africa') . ' ' . $api_check['code'] . '</i>)';
                }
        ...
        private function check_cached_token() {
            ...
            if ( ! $token ) {
                ...
                if ( $result === null ) {
                    ...
                    return $this->render_error(__('Found, but does not work. Deleted.', 'woo-topship-africa'));
                }
                return $this->render_success(__('Found and works.', 'woo-topship-africa'));
            }
        }

  ...
  class Admin {
      ...
      <div class="notice notice-info pakettikauppa-notice pakettikauppa-notice--setup">
        ...
        <div class="pakettikauppa-notice__content">
          <p>
            ...
            printf(esc_html__('Thank you for installing %s! To get started smoothly, please open our setup wizard.', 'woo-topship-africa'), $this->core->vendor_fullname);
            ?>
    ...
    public function ajax_meta_box() {
      ...
      if ( ! is_numeric($_POST['post_id']) ) {
        $this->add_admin_notice(__('Received Post ID is not a number', 'woo-topship-africa'), 'error');
        wp_die('', '', 501);
      }
    ...
    public function register_multi_create_orders( $bulk_actions ) {
      ...
      if ( version_compare($wp_version, '5.6.0', '>=') ) {
        $bulk_actions[$this->core->vendor_name] = array(
          $this->core->params_prefix . 'create_multiple_shipping_labels' => __('Create and fetch shipping labels', 'woo-topship-africa'),
          $this->core->params_prefix . 'create_custom_shipments' => __('Create custom shipments', 'woo-topship-africa'),
        );
      } else {
        $bulk_actions[$this->core->params_prefix . 'create_multiple_shipping_labels'] = $this->core->vendor_name . ': ' . __('Create and fetch shipping labels', 'woo-topship-africa');
        $bulk_actions[$this->core->params_prefix . 'create_custom_shipments'] = $this->core->vendor_name . ': ' . __('Create custom shipments', 'woo-topship-africa');
      }
    ...
    public function register_quick_create_order( $order ) {
      ...
      $actions = array(
        'name'   => __('Create shipping label', 'woo-topship-africa'),
    ...
    public function create_multiple_shipments( $redirect_to ) {
      ...
      if ( $contents->{'response.file'}->__toString() === '' ) {
        $this->add_admin_notice(__('Cannot find shipments with given shipment numbers.', 'woo-topship-africa'), 'error');
    ...
    public function wc_pakettikauppa_updated() {
      ...
      if ( ! $shipping_method_found ) {
        ...
        /* translators: %s: Vendor full name */
        echo '<p>' . sprintf(__('%s has been installed/updated but no shipping methods are currently active!', 'woo-topship-africa'), $this->core->vendor_fullname) . '</p>';
        echo '</div>';
      }
    }
    ...
    public function add_error_notice( $message, $show_prefix_text = true ) {
      if ( ! empty($message) ) {
        ...
        if ( $show_prefix_text ) {
          ...
          $print_error = wp_sprintf(__('An error occurred: %s', 'woo-topship-africa'), $message);
        } else {
          $print_error = $message;
        }
    ...
    public function add_success_notice( $message, $show_prefix_text = true ) {
      if ( ! empty($message) ) {
        ...
        if ( $show_prefix_text ) {
          /* translators: %s: Error message */
          $print_error = wp_sprintf(__('Succeed: %s', 'woo-topship-africa'), $message);
        } else {
          $print_error = $message;
        }
        printf('<div class="%1$s"><p>%2$s</p></div>', esc_attr($class), esc_html($print_error));
      ...
      if ( $file === $this->core->basename ) {
        $row_meta = array(
          'service' => sprintf(
            ...
            esc_attr__('Visit Website', 'woo-topship-africa'),
            /* translators: %s: Vendor name */
            sprintf(esc_html__('Show site %s', 'woo-topship-africa'), $this->core->vendor_name)
          ),
        );
    ...
    public function register_meta_boxes() {
      foreach ( wc_get_order_types('order-meta-boxes') as $type ) {
        add_meta_box(
          'woo-topship-africa', // Using a variable WILL BREAK JS
          // $this->core->prefix,
     ...
     * the WordPress hook plugin_action_links_woo-topship-africa.
     *
    ...
    public function show_pickup_point_in_admin_order_meta( $order ) {
      ...
      $services_data = array(
        'lqweight' => array(
          'title' => esc_attr__('Total weight of dangerous goods', 'woo-topship-africa'),
      ...
      <h4>
        <?php
        /* translators: %s: Vendor name */
        printf(esc_attr__('%s Shipping', 'woo-topship-africa'), $this->core->vendor_name);
        ?>
      </h4>
      <div class="address pakettikauppa">
        <p class="form-field pakettikauppa-field">
          <strong><?php esc_attr_e('Requested pickup point', 'woo-topship-africa'); ?></strong>
          <?php
      ...
      <div class="edit_address pakettikauppa">
        ...
        <p class="form-field <?php echo $field_key; ?>">
          <label for="<?php echo $field_key; ?>"><?php esc_attr_e('Phone', 'woo-topship-africa'); ?></label>
        ...
        <p class="form-field <?php echo $field_key; ?>">
          <label for="<?php echo $field_key; ?>"><?php esc_attr_e('Email', 'woo-topship-africa'); ?></label>
      ...
      <?php if ( ! empty($label['tracking_code']) ) : ?>
        <div class="pakettikauppa-shiplabel pakettikauppa-design-foldedcorner">
          ...
          <p>
            ...
            <?php if ( $manifest_id ) : ?>
              <strong><?php echo __('Pickup order', 'woo-topship-africa'); ?>:</strong> <span>#<?php echo $manifest_id; ?></span><br />
              <?php
                $manifest = get_post($manifest_id);
                $pickup_order_status = get_post_status_object(get_post_status($manifest))->label;
              ?>
              <strong><?php echo __('Pickup order status', 'woo-topship-africa'); ?>:</strong> <span><?php echo $pickup_order_status; ?></span><br />
            <?php endif; ?>
            <strong><?php echo __('Status', 'woo-topship-africa'); ?>:</strong> <span><?php echo esc_attr(Shipment::get_status_text($label['shipment_status'])); ?></span><br />
            <?php if ( ! empty($label['label_code']) ) : ?>
              <strong><?php echo __('Label code', 'woo-topship-africa'); ?>:</strong> <span><?php echo $label['label_code']; ?></span><br />
            <?php endif; ?>
            <?php if ( ! empty($label['pickup_id']) ) : ?>
              <strong><?php echo __('Pickup point', 'woo-topship-africa'); ?>:</strong> <span><?php echo $label['pickup_name']; ?></span><br />
            <?php endif; ?>
            <?php if ( ! empty($label['additional_services']) ) : ?>
              ...
              <?php if ( ! empty($services) ) : ?>
                <strong><?php echo __('Services', 'woo-topship-africa'); ?>:</strong> <span><?php echo $services; ?></span><br />
              <?php endif; ?>
            <?php endif; ?>
            <?php if ( ! empty($label['products']) ) : ?>
              ...
              foreach ( $label['products'] as $prod ) {
                ...
                $product_name = ($product) ? $product->get_title() : __('Unknown product', 'woo-topship-africa') . ' (ID: ' . $prod['prod'] . ')';
                $products .= '<br/>' . $prod['qty'] . ' x ' . $product_name;
              }
              ?>
              <strong><?php echo __('Products', 'woo-topship-africa'); ?>:</strong> <span><?php echo $products; ?></span><br />
            <?php endif; ?>
            <br />
            <a href="<?php echo esc_url($label['document_url']); ?>" target="_blank" class="download"><?php esc_attr_e('Print', 'woo-topship-africa'); ?></a> -
            <?php if ( ! empty($label['tracking_url']) ) : ?>
              <a href="<?php echo esc_url($label['tracking_url']); ?>" target="_blank" class="tracking"><?php esc_attr_e('Track', 'woo-topship-africa'); ?></a> -
            <?php endif; ?>
            <a href="javascript:void(0)" class="status" name="wc_pakettikauppa[get_status]" data-value="<?php echo esc_attr($label['tracking_code']); ?>" onclick="pakettikauppa_meta_box_submit(this);"><?php echo __('Refresh', 'woo-topship-africa'); ?></a> -
            <?php if ( $this->core->order_pickup ) : ?>
            <a href="javascript:void(0)" class="manifest" name="wc_pakettikauppa[add_to_manifest]" data-value="<?php echo esc_attr($label['tracking_code']); ?>" onclick="pakettikauppa_meta_box_submit(this);"><?php echo __('Add to pickup order', 'woo-topship-africa'); ?></a> -
            <?php endif; ?>
            <a href="javascript:void(0)" class="delete" name="wc_pakettikauppa[delete_shipping_label]" data-value="<?php echo esc_attr($label['tracking_code']); ?>" onclick="pakettikauppa_meta_box_submit(this);"><?php echo __('Delete', 'woo-topship-africa'); ?></a>
          </p>
        </div>
      <?php endif; ?>
      <?php
    ...
    private function tpl_return_label( $label ) {
      ...
      <div class="pakettikauppa-returnlabel pakettikauppa-design-foldedcorner">
        ...
        <p>
          ...
          <a href="<?php echo esc_url($label['document_url']); ?>" target="_blank" class="download"><?php esc_attr_e('Print', 'woo-topship-africa'); ?></a> -
          <a href="<?php echo esc_url($label['tracking_url']); ?>" target="_blank" class="tracking"><?php esc_attr_e('Track', 'woo-topship-africa'); ?></a> -
          <a href="javascript:void(0)" class="delete" name="wc_pakettikauppa[delete_return_label]" data-value="<?php echo esc_attr($label['tracking_code']); ?>" onclick="pakettikauppa_meta_box_submit(this);"><?php echo __('Delete', 'woo-topship-africa'); ?></a>
        </p>
      </div>
      <?php
    }
    ...
    private function tpl_return_label_global_buttons() {
      ...
      <div class="pakettikauppa-global-labels-buttons">
        <button type="button" value="create_return_label" name="wc_pakettikauppa[create_return_label]" onclick="pakettikauppa_meta_box_submit(this);" class="button pakettikauppa_meta_box"><?php echo __('Create Return Label', 'woo-topship-africa'); ?></button>
      </div>
      <?php
    }
    ...
    private function tpl_products_selector( $order ) {
      ...
      <div class="pakettikauppa-metabox-products">
        ...
        ?>
        <h4><?php echo __('Create for products', 'woo-topship-africa'); ?></h4>
    ...
    public function meta_box( $post ) {
      ...
      if ( ! Shipment::validate_order_shipping_receiver($order) ) {
        esc_attr_e('Please add shipping info to the order to manage shipments.', 'woo-topship-africa');
      ...
      <div>
        <?php if ( $show_section === 'custom' ) : ?>
          <div class="pakettikauppa-notice notice-error">
            <p>
              <?php _e('No shipping method configured! Configure shipping method from settings.', 'woo-topship-africa'); ?>
            </p>
          </div>
        <?php endif; ?>
        ...
        <?php
        if ( empty($service_id) ) {
          $this->tpl_section_title(__('Send order', 'woo-topship-africa'));
        }
        if ( ! empty($labels) ) {
          $this->tpl_section_title(__('Shipping labels', 'woo-topship-africa'));
          foreach ( $labels as $label ) {
            $this->tpl_shipping_label($label, $post->ID);
          }
        }
        if ( (! empty($labels) || ! empty($return_shipments)) && ! empty($service_id) ) {
          $this->tpl_section_title(__('Return labels', 'woo-topship-africa'));
        ...
        ?>
          <div class="pakettikauppa-services">
            ...
            <fieldset class="pakettikauppa-metabox-fieldset" id="wc_pakettikauppa_shipping_method" style="<?php echo $show_main; ?>">
              ...
              <?php if ( ! empty($default_additional_services) ) : ?>
                <h4><?php echo esc_attr__('Additional services', 'woo-topship-africa'); ?>:</h4>
                <ol style="list-style: circle;">
                  ...
                  <?php if ( in_array('3102', $default_additional_services, true) ) : ?>
                    <li>
                      <?php echo esc_html__('Parcel count', 'woo-topship-africa'); ?>:
              ...
              <?php if ( $pickup_point_id ) : ?>
                ...
                <div class="pakettikauppa-pickup-point-requested">
                  <h4>
                    <?php echo esc_html__('Requested pickup point', 'woo-topship-africa'); ?>
                  </h4>
            ...
            <fieldset class="pakettikauppa-metabox-fieldset" id="wc_pakettikauppa_custom_shipping_method" style="<?php echo $show_custom; ?>">
              ...
              <select name="wc_pakettikauppa_service_id" id="pakettikauppa-service" class="pakettikauppa_metabox_values" onchange="pakettikauppa_change_shipping_method(this);">
                <option value="__NULL__"><?php esc_html_e('No shipping', 'woo-topship-africa'); ?></option>
              ...
              <?php else : ?>
                ...
                $message = sprintf(__('Service not working. Please check <a href="%s">settings</a>.', 'woo-topship-africa'), $settings_url);
                ?>
              ...
              <?php foreach ( $all_additional_services as $method_code => $_additional_services ) : ?>
                ...
                <?php if ( $this->shipment->service_has_pickup_points($method_code) ) : ?>
                  ...
                  $select_first_option = '- ' . __('Select', 'woo-topship-africa') . ' -';
                  $settings = $this->shipment->get_settings();
                  ...
                  <div id="pickup-changer-<?php echo $method_code; ?>" class="pakettikauppa-pickup-changer" style="display: none;">
                    ...
                    <div class="pakettikauppa-pickup-search">
                      <h4><?php echo __('Search pickup points', 'woo-topship-africa'); ?></h4>
                        ...
                        <ol style="list-style:circle;">
                          <li>
                            <input
                                    ...
                                    value="all"
                                    />
                            <label for="search_filter_all_<?php echo $method_code; ?>"><?php echo __('Without filters', 'woo-topship-africa'); ?></label>
                          </li>
                          <li>
                            <input
                                    type="radio"
                                    ...
                                    />
                            <label for="search_filter_filters_<?php echo $method_code; ?>"><?php echo __('With filters', 'woo-topship-africa'); ?></label>
                          </li>
                        </ol>
                       <?php } ?>
                      <button type="button" value="search" class="button button-small btn-search" onclick="pakettikauppa_pickup_points_by_custom_address(btn_values_<?php echo $method_code; ?>);"><?php echo __('Search', 'woo-topship-africa'); ?></button>
                      <span class="pakettikauppa-msg-error error-pickup-search" style="display:none;"><?php echo __('No pickup points were found', 'woo-topship-africa'); ?></span>
                    </div>
                    <div class="pakettikauppa-pickup-select-block">
                      <h4><?php echo __('Select pickup point', 'woo-topship-africa'); ?></h4>
          ...
          <div class="pakettikauppa-general">
            ...
            <?php if ( $this->core->shippingmethod == 'pakettikauppa_shipping_method' ) : ?>
              <div class="pakettikauppa-estimated-price">
                <span class="title">
                  <?php echo esc_html__('Estimated shipping price', 'woo-topship-africa'); ?>:
                </span>
          ...
          <p class="pakettikauppa-metabox-footer">
            ...
            <label for="wc_pakettikauppa_add_to_manifest" id="custom_add_to_manifest">
              ...
              <?php echo esc_html__('Add to pickup order', 'woo-topship-africa'); ?>
            </label>
            <?php endif; ?>
            <?php if ( ! empty($service_id) ) : ?>
              <?php $button_text = __('Custom shipping...', 'woo-topship-africa'); ?>
              <button type="button" value="change" id="pakettikauppa_metabtn_change" class="button pakettikauppa_meta_box" onclick="pakettikauppa_change_method(this);" data-txt1="<?php echo $button_text; ?>" data-txt2="<?php echo __('Original shipping...', 'woo-topship-africa'); ?>">
            ...
            <button type="button" value="create" id="pakettikauppa_metabtn_create" name="wc_pakettikauppa[create]" class="button pakettikauppa_meta_box button-primary" onclick="pakettikauppa_meta_box_submit(this);">
              <?php echo __('Create', 'woo-topship-africa'); ?>
            </button>
          </p>
      </div>
      <?php
    ...
    private function create_return_label( \WC_Order $order ) {
      ...
      if ( ! $shipping_label ) {
        $this->add_error_notice(esc_attr__('It is not allowed to create a return label when shipping labels not exists', 'woo-topship-africa'));
        return;
      }
      ...
      switch ( $service_provider ) {
        ...
        case 'Matkahuolto':
        default:
          $order->add_order_note(__('Unable to create return label for this shipment type.', 'woo-topship-africa'));
          return;
      }
    ...
    private function delete_shipping_label( \WC_Order $order, $tracking_code ) {
      try {
        ...
        $order->add_order_note(sprintf(esc_attr__('Deleted %1$s shipping label %2$s.', 'woo-topship-africa'), $this->core->vendor_name, $tracking_code));
      } catch ( \Exception $e ) {
        ...
        add_action(
          ...
          function() use ( $e ) {
            /* translators: %s: Error message */
            $this->add_error_notice(wp_sprintf(esc_attr__('An error occurred: %s', 'woo-topship-africa'), $e->getMessage()));
          }
        );
        ...
        $order->add_order_note(
          sprintf(
            ...
            esc_attr__('Deleting %1$s shipment failed! Errors: %2$s', 'woo-topship-africa'),
      ...
      try {
        ...
        foreach ( $return_shipments as $return_shipment ) {
          if ( $return_shipment['tracking_code'] === $tracking_code ) {
            ...
            $order->add_order_note(sprintf(esc_attr__('Deleted %1$s return label %2$s.', 'woo-topship-africa'), $this->core->vendor_name, $tracking_code));
            return;
          }
        }
      } catch ( \Exception $e ) {
        ...
        $order->add_order_note(
          sprintf(
            /* translators: %1$s: Vendor name, %2$s: Error message */
            esc_attr__('Deleting %1$s return label failed! Errors: %2$s', 'woo-topship-africa'),
            $this->core->vendor_name,
            $e->getMessage()
          )
        );
    ...
    public function show() {
      ...
      if ( empty( $_REQUEST['tracking_code'] ) ) { // @codingStandardsIgnoreLine
        esc_attr_e('Shipment tracking code is not defined.', 'woo-topship-africa');
      ...
      try {
        ...
        esc_attr_e('Failed to get shipment label.', 'woo-topship-africa');
        echo '</br>' . esc_attr__('Error', 'woo-topship-africa') . ': ' . $e->getMessage();

        return;
      }

      if ( $contents->{'response.file'}->__toString() === '' ) {
        esc_attr_e('Cannot find shipment with given shipment number.', 'woo-topship-africa');
    ...
    public function get_custom_shipment_table() {
      ...
      <div id="pakettikauppa-shipments-table">
        <div>
          <h3><?php echo esc_attr__('Pakettikaupa create custom shipments', 'woo-topship-africa'); ?></h3>
          <div>
            <table class="wp-list-table widefat fixed striped table-view-list posts">
              <thead>
                <tr>
                  <th><?php echo esc_attr__('Order', 'woo-topship-africa'); ?></th>
                  <th><?php echo esc_attr__('Current shipping method', 'woo-topship-africa'); ?></th>
                  <th><?php echo esc_attr__('New shipping method', 'woo-topship-africa'); ?></th>
                </tr>
              </thead>
              <tbody>
                <?php
                  foreach ( $ids as $id ) {
                    ...
                    if ( $order ) {
                ?>
                      <tr class="inside" id="woo-topship-africa_<?php echo $id; ?>">
        ...
        <button type="button" value="create" id="pakettikauppa_metabtn_create_bulk" name="wc_pakettikauppa[create]" class="button pakettikauppa_meta_box button-primary" onclick="pakettikauppa_meta_box_bulk_submit(this);">
          <?php echo esc_attr__('Create', 'woo-topship-africa'); ?>
        </button>
      </div>
      <?php
    }
    ...
    public function get_current_shipment( $order ) {
      ...
      <fieldset class="pakettikauppa-metabox-fieldset" id="wc_pakettikauppa_shipping_method">
        ...
        <?php if ( $pickup_point_id ) : ?>
          ...
          <div class="pakettikauppa-pickup-point-requested">
            <p>
              <b><?php echo esc_html__('Pickup point', 'woo-topship-africa'); ?></b></br>
              <?php echo esc_html($pickpoint_requested); ?>
            </p>
          </div>
        <?php endif; ?>
      ...
      foreach ( $all_additional_services as $method_code => $_additional_services ) {
        if ( $this->shipment->service_has_pickup_points($method_code) ) {
          ...
          <div id="pickup-changer-<?php echo $method_code; ?>" class="pakettikauppa-pickup-changer" <?php echo $service_id != $method_code ? 'style="display: none;"' : ''; ?>>
            ...
            <div class="pakettikauppa-pickup-select-block">
              <p style="margin-bottom: 5px;"><?php echo __('Select pickup point', 'woo-topship-africa'); ?></p>
              <select class="pakettikauppa_metabox_values pakettikauppa-pickup-select" onchange="pakettikauppa_change_selected_pickup_point(this);">
                <?php if ( is_array($pickup_points) ) : ?>
                  <?php foreach ( $pickup_points as $point ) : ?>
                    <?php

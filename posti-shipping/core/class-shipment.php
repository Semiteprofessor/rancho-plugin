  ...
  class Shipment {
    ...
    public static function get_status_text( $status_code ) {
      switch ( intval($status_code) ) {
        case 13:
          $status = __('Item is collected from sender - picked up', 'woo-topship-africa');
          break;
        case 20:
          $status = __('Exception', 'woo-topship-africa');
          break;
        case 22:
          $status = __('Item has been handed over to the recipient', 'woo-topship-africa');
          break;
        case 31:
          $status = __('Item is in transport', 'woo-topship-africa');
          break;
        case 38:
          $status = __('C.O.D payment is paid to the sender', 'woo-topship-africa');
          break;
        case 45:
          $status = __('Informed consignee of arrival', 'woo-topship-africa');
          break;
        case 48:
          $status = __('Item is loaded onto a means of transport', 'woo-topship-africa');
          break;
        case 56:
          $status = __('Item not delivered – delivery attempt made', 'woo-topship-africa');
          break;
        case 68:
          $status = __('Pre-information is received from sender', 'woo-topship-africa');
          break;
        case 71:
          $status = __('Item is ready for delivery transportation', 'woo-topship-africa');
          break;
        case 77:
          $status = __('Item is returning to the sender', 'woo-topship-africa');
          break;
        case 91:
          $status = __('Item is arrived to a post office', 'woo-topship-africa');
          break;
        case 99:
          $status = __('Outbound', 'woo-topship-africa');
          break;
        default:
          $status = __('Unknown status', 'woo-topship-africa');
    ...
    public function check_api_credentials( $account_number, $secret_key ) {
      ...
      $status = array(
        'api_good' => true,
        'msg' => __('API is good', 'woo-topship-africa'),
        'code' => '',
        'error' => '',
      );
      ...
      if ( empty($account_number) || empty($secret_key) ) {
        ...
        $status['msg'] = __('Bad API key or API secret', 'woo-topship-africa');
      } else {
        try {
          ...
          if ( ! empty($configs['production']['use_posti_auth']) ) {
            ...
            if ( empty($token) ) {
              $status['api_good'] = false;
              $status['msg'] = __('Failed to connect with server', 'woo-topship-africa');
              ...
              if ( empty($checker) ) {
                $status['api_good'] = false;
                $status['msg'] = __('Failed to check API credentials or them are bad', 'woo-topship-africa');
                $status['error'] = (! empty($this->client->http_error)) ? $this->client->http_error : '';
              }
            }
          } else {
          ...
          $status['api_good'] = false;
          $status['msg'] = __('An error occurred while checking API credentials', 'woo-topship-africa');
        }
    ...
    public function create_shipment( \WC_Order $order, $service_id = null, $additional_services = null, $selected_products = array(), $extra_params = array() ) {
      ...
      if ( empty($service_id) || $service_id === '__NULL__' || $service_id === '__PICKUPPOINTS__' ) {
        ...
        $order->add_order_note(esc_attr__('The shipping label was not created because the order does not contain valid shipping method.', 'woo-topship-africa'));
        ...
        add_action(
          ...
          function() {
            echo '<div class="update-nag notice">' .
                esc_attr__('The shipping label was not created because the order does not contain valid shipping details.', 'woo-topship-africa') .
                '</div>';
          }
        );
      ...
      try {
        ...
        $order->add_order_note(sprintf(esc_attr__('Failed to create shipment. Errors: %s', 'woo-topship-africa'), $e->getMessage()));
        add_action(
          'admin_notices',
          function() use ( $e ) {
            /* translators: %s: Error message */
      ...
      if ( $tracking_code === null ) {
        $this->add_error('error');
        $order->add_order_note(esc_attr__('Failed to create shipment.', 'woo-topship-africa'));
        add_action(
          'admin_notices',
          function() {
            /* translators: %s: Error message */
            $this->add_error_notice(esc_attr__('Failed to create shipment.', 'woo-topship-africa'));
          }
        );
      ...
      // Add order note
      $dl_link       = sprintf('<a href="%1$s" target="_blank">%2$s</a>', $document_url, esc_attr__('Print document', 'woo-topship-africa'));
      $tracking_link = sprintf('<a href="%1$s" target="_blank">%2$s</a>', $tracking_url, __('Track', 'woo-topship-africa'));
      ...
      $order->add_order_note(
        sprintf(
          ...
          __('Created %1$s shipment.<br>%2$s<br>%1$s - %3$s<br>%4$s', 'woo-topship-africa'),
      ...
      if ( ! empty($settings['post_label_to_url']) ) {
        if ( $this->post_label_to_url($settings['post_label_to_url'], $tracking_code) === false ) {
          $this->add_error('error');
          $order->add_order_note(__('Posting label to URL failed!', 'woo-topship-africa'));

          return null;
        } else {
          $order->add_order_note(__('Label posted to URL successfully.', 'woo-topship-africa'));
        }
      }
            ...
            add_action(
              ...
              function() use ( $token ) {
                ...
                if ( $_GET['page'] === 'wc-settings' && $_GET['tab'] === 'shipping' ) {
                  $message = (isset($token->message)) ? $token->message : __('Unknown error', 'woo-topship-africa');
                  echo '<div class="notice notice-error"><p><b>' . $this->core->vendor_fullname . ' ' . __('error', 'woo-topship-africa') . ':</b> ' . $message . '</p></div>';
                }
              }
            );
    ...
    public function create_shipment_from_order( $order, $service_id = null, $additional_services = array(), $selected_products = array(), $extra_params = array() ) {
      ...
      try {
        ...
        /* translators: %s: Error message */
        throw new \Exception(wp_sprintf(__('Tracking code creation failed: %s', 'woo-topship-africa'), $e->getMessage()));
      }

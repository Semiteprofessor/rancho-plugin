...
if ( ! class_exists(__NAMESPACE__ . '\Frontend') ) {
  ...
  class Frontend {
    ...
    public function __construct( Core $plugin ) {
      // $this->id = 'woo-topship-africa'; // not used for anything
      $this->core = $plugin;
    }
    ...
    public function display_error( $error = null ) {
      if ( ! $error ) {
        $error = __('An error occurred. Please try again later.', 'woo-topship-africa');
      }
    ...
    */
    public function pickup_point_field_html() {
      ...
      if ( empty($shipping_postcode) ) {
        $error_msg = esc_attr__('Empty postcode. Please check your address information.', 'woo-topship-africa');
      } else if ( $shipping_country == 'FI' && ! is_numeric($shipping_postcode) ) {
        $error_msg = sprintf(
        /* translators: %s: Postcode */
          esc_attr__('Invalid postcode "%1$s". Please check your address information.', 'woo-topship-africa'),
    ...
    private function process_pickup_points_to_option_array( $pickup_points ) {
      $options_array = array( '' => array( 'text' => '- ' . __('Select a pickup point', 'woo-topship-africa') . ' -' ) );
      ...
      $options_array['other'] = array(
        'text' => __('Other', 'woo-topship-africa'),
        //'is_private' => $value->point_type === 'PRIVATE_LOCKER',
      );
    ...
    public function validate_checkout() {
      ...
      if ( ! wp_verify_nonce(sanitize_key($_POST['woocommerce-process-checkout-nonce']), 'woocommerce-process_checkout') ) {
        ...
        //$this->add_error(__('We were unable to process your order, please try again.', 'woo-topship-africa'));
        //return;
      }
      ...
      if ( $pickup_data === '__NULL__' || $pickup_data === '' || $pickup_data === 'other' ) {
        ...
        if ( $shipping_needs_pickup_points ) {
          $this->add_error(__('Please choose a pickup point.', 'woo-topship-africa'));
        }

...
jQuery(function ($) {
  window.pakettikauppa_meta_box_submit = function (obj) {
    ...
    if ($("#wc_pakettikauppa_shipping_method").is(":visible")) {
      ...
      data["additional_text"] = $(
        "textarea.pakettikauppa-additional-info"
      ).val();

      if ($("#pickup-changer-" + shipping_method).length) {
        data["custom_pickup"] = $(
          "#pickup-changer-" + shipping_method + " .pakettikauppa-pickup-select"
        )
          .find(":selected")
          .data("id");
      }
      ...
      $(
        "#pk-admin-additional-services-" +
          shipping_method +
          " .pakettikauppa_metabox_values"
      ).each(function (i, obj) {
        var name = $(obj).attr("name");
        data[name] = $(this).val();
      });
      ...
      $(
        "#pk-admin-additional-services-" +
          shipping_method +
          " .pakettikauppa_metabox_array_values"
      ).each(function (i, obj) {
    ...
    $.post(woocommerce_admin_meta_boxes.ajax_url, data, function (response) {
      $("#woo-topship-africa .inside").html(response);
      $("#woo-topship-africa").unblock();
    }).fail(function () {
      location.reload();
    });
  };
  ...
  window.pakettikauppa_meta_box_bulk_submit = function (obj) {
    ...
    var ids = [];
    $("#pakettikauppa-modal")
      .find("input[name='pakettikauppa_order_id[]']")
      .each(function () {
        ids.push($(this).val());
      });
    ...
    if (ids.length > 0) {
      ids.forEach(function (id) {
        var data = {
          ...
          post_id: id,
          security: $(
            "#woo-topship-africa_" + id + " #pakettikauppa_metabox_nonce"
          ).val(),
          request_id: $(
            "#woo-topship-africa_" + id + " #pakettikauppa_microtime"
          ).val(),
        };

        var shipping_method = $(
          "#woo-topship-africa_" + id + " #pakettikauppa-service"
        ).val();
        data["wc_pakettikauppa_service_id"] = shipping_method;
        data["custom_method"] = 1;

        if (
          $("#woo-topship-africa_" + id + " #pickup-changer-" + shipping_method)
            .length
        ) {
          data["custom_pickup"] = $(
            "#woo-topship-africa_" +
        ...
        $.post(
          ...
          data,
          function (response) {
            $("#woo-topship-africa_" + id + ".inside td:last-child").remove();
            $("#woo-topship-africa_" + id + ".inside td:last-child").remove();
            $("#woo-topship-africa_" + id + ".inside").append(response);
  ...
  window.pakettikauppa_change_method = function (obj) {
    ...
    if (
      $($(obj).closest("div"))
        .find("#wc_pakettikauppa_shipping_method")
        .is(":visible")
    ) {
      $($(obj).closest("div"))
        .find("#wc_pakettikauppa_shipping_method")
        .slideUp("slow");
      $($(obj).closest("div"))
        .find("#wc_pakettikauppa_custom_shipping_method")
        .slideDown("slow");
  ...
  window.pakettikauppa_change_shipping_method = function (obj) {
    ...
    $($(obj).closest(".pakettikauppa-metabox-fieldset"))
      .find(".pakettikauppa-pickup-changer")
      .each(function (i, obj) {
        $(this).hide();
      });
    ...
    $($(obj).closest(".pakettikauppa-metabox-fieldset"))
      .find("#pickup-changer-" + selectedService)
      .show();
    pakettikauppa_trigger_pickup_list(selectedService);

    var element = document.querySelector(
      ".prod_select_dropdown .content .quantity"
    );
    if (element) {
  ...
  window.pakettikauppa_pickup_points_by_custom_address = function (values) {
    var address = $("#" + values.container_id)
      .find(".pakettikauppa-pickup-search-field")
      .val();
    var method = $("#" + values.container_id)
      .find(".pakettikauppa-pickup-method")
      .val();
    var select_field = $("#" + values.container_id).find(
      ".pakettikauppa-pickup-select"
    );
    var pickup_point_type = $("#" + values.container_id)
      .find('input[name="wc_pakettikauppa_search_filter"]:checked')
      .val();

    $("#" + values.container_id)
      .find(".error-pickup-search")
      .hide();
  ...
  $(document).on(
    "change",
    "ol.pk-admin-additional-services input",
    function () {
      var element = document.querySelector(
        ".prod_select_dropdown .content .quantity"
      );
...
function init_prod_select() {
  var txt = document.getElementById("prod_select_droptxt"),
    ...
    list = document.querySelectorAll(
      '.prod_select_dropdown .content input[type="checkbox"]'
    ),
...
function update_prod_select(list, quantity, txt) {
  for (var i = 0, arr = []; i < list.length; i++) {
    if (list[i].checked)
      arr.push(quantity[i].value + " x " + list[i].getAttribute("data-name"));
  }
...
function update_lqweight_span(list, quantity) {
  ...
  var all_spans = document.querySelectorAll(
    ".pakettikauppa-services .changeable_lqweight"
  );
...
function update_estimated_price(list, quantity) {
  ...
  if (!jQuery("#wc_pakettikauppa_shipping_method").is(":visible")) {
    ...
    var all_services = document
      .getElementById("pk-admin-additional-services-" + selected_method)
  ...
  var selected_point = "";
  var select_field = document.querySelector(
    "#pickup-changer-" + selected_method + " .pakettikauppa-pickup-select"
  );
